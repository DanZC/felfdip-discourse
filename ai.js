...
function getActiveMembers(sstate) {
    var activeMembers = [];
    console.log(`members.length = ${sstate.members.length}`);
    for(var n in sstate.members) {
...
function getAttackingMembers(sstate) {
    var activeMembers = [];
    for(var n in sstate.members) {
        var c = sstate.members[n];
        var cinfo = getInfo(c.index);
...
function hasActiveMemberCard(sstate) {
    ...
    if(!isEmpty(sstate.members[i])) {
        if(getInfo(sstate.members[i].index).type === CardType.MEMBER)
            return true;
    }
    return false;
}
    ...
    function doAI(state) {
        ...
        case DuelPhase.ACTION:
            ...
            for(n in state.self.hand) {
                ...
                var cinfo = getInfo(card.index);
                if(cinfo.type === CardType.MEMBER) {
                    if(cinfo.lvl <= 1 && !flags.playedMemberCard) {
                        var c = getAvailableMemberSlot(state.self);
                        if(c != -1) {
                            moves.push(`PLAY ${n} MBR${c}`);
                            flags.playedMemberCard = true;
                            return moves;
                        }
                    }
                }
        ...
        case DuelPhase.BATTLE:
            ...
            var members = getAttackingMembers(state.self);
            for(mb in members) {
                var n = members[mb];
                ...
                //console.log(`Can I destroy a member card with my attack?`)
                var opps = getActiveMembers(state.opponent);
                var tgt = null;
                console.log(`len=${opps.length}`)
                for(var mm in opps) {
                    var nn = opps[mm];
                    var cc = state.opponent.members[nn];
                    var ccinfo = getInfo(cc.index);
                    if(calcDamage(atk, ccinfo.def) > ccinfo.hp) {
                        //console.log(`ATTACK MBR${n} MBR${nn}`)
                        moves.push(`ATTACK MBR${n} MBR${nn}`);
                        tgt = nn;
                        return moves;
                    }
                }
               ...
               //     continue;
                ...
                if(pref !== -1) {
                    //console.log(`ATTACK MBR${n} MBR${pref}`)
                    moves.push(`ATTACK MBR${n} MBR${pref}`);
                    return moves;
                }
            }
