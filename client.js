...
Client.getEventName = function() {
	if(Client.events.length > 0) {
		var ev = Client.events[0];
		return ev['name'];
	} else {
		return "";
	}
...
Client.shiftEvents = function() {
	if(Client.events.length > 0) {
		Client.events.shift();
	}
}

Client.pushEvent = function(name, data) {
	var ev = {'name': name, 'data': data};
